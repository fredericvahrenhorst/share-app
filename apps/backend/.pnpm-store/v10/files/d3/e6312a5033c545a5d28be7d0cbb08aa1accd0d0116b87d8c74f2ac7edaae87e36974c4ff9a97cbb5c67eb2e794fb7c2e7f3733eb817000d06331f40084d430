...
export const populateFolderDataEndpoint = {
    handler: async (req)=>{
        ...
        if (!(req.payload.config.folders && Boolean(req.payload.collections?.[req.payload.config.folders.slug]))) {
            return Response.json({
        ...
        const folderCollectionConfig = req.payload.collections?.[req.payload.config.folders.slug]?.config;
