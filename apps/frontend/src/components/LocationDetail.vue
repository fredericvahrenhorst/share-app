<template>
    ...
    >
        ...
        <!-- <ion-content class="ion-padding"> -->
            <div v-if="popupLocation" class="relative p-4 max-h-[600px] overflow-scroll">
                <!-- Kopfbereich -->
                <div class="flex items-center mb-2">
                    <ion-icon :icon="categoryIcon" class="text-xl mr-2" />
                    <span class="text-sm text-gray-500">{{ popupLocation.category?.name }}</span>
                    <button
                        @click="locationModal.$el.dismiss()"
                        class="ml-auto text-gray-400 hover:text-gray-600 text-xl"
                    >
                        &times;
                    </button>
                </div>
                <h2 class="text-2xl font-bold mb-1">{{ popupLocation.name }}</h2>
                <div class="flex items-center mb-2">
