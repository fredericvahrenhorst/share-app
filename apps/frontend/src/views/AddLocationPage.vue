...
onMounted(() => {
    ...
    if (categories.value.length === 0) {
        locationsStore.getAllLocations(); // Lädt auch Kategorien indirekt, besser wäre fetchCategories
    }
});
...
async function submitLocation() {
    ...
    try {
        ...
        if (selectedFile.value) {
            ...
            // Payload erwartet 'alt' Text für Media
            formData.append('alt', formData.value.name);
            ...
            const mediaResponse = await apiCall('/api/media', {
                method: 'POST',
                data: formData,
                headers: { 'Content-Type': 'multipart/form-data' } // Wird von axios überschrieben, aber als Marker nützlich
            });
